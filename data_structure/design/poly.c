#include<stdio.h>
 ...
 struct poly p1[10],p2[10],rp[10]; // p1 and p2 are polynomial arrays, rp is the result

 int readPoly(struct poly[]);
 int addPoly(struct poly[], struct poly[], int, int, struct poly[]);
 int subPoly(struct poly[], struct poly[], int, int, struct poly[]);
 void printPoly( struct poly [],int terms);
 ...
 int main()
 {
	...
	// add the polynomials and print the result
 ...
 void printPoly(struct poly p[10],int term)
 {
	int k;
