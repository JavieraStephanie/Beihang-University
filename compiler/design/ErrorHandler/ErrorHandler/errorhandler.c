...
FILE* fi, * fo;
size_t templen;
bool tempExp = false, ret = false, arraysize = false, hasvar = false, mainFunc = false, neg = false, assign = false, valfunc = true, idenvar = false;
int id = -1, varnum = 0, paramnum = 0, newline = 0, lc = 1, l = 0, r = 0, len = 0, str = 0, num = 0, num2 = 0, intcon = 0, layer = 0, funccounter = 0, valcounter = 0;
int funcid[10], fid = 0, idenid[10], iid = 0, intExp[10], iei = 0;
char* subStr, tempstr[100], line[200], chstr[2], op[5] = "PLUS", op2[5] = "\0", funcname[20] = "\0", functype[10] = "\0", idenname[20] = "\0", identype[10] = "\0";
		...
		if (isSymbol(line[r]) && l == r) {
			...
			if (strcmp(checkSym(line[r - 1]), "QUOTE") && str != 0) {
				...
				if (!strcmp(checkSym(line[r - 1]), "QUOTE") && str == 0) {
					...
					if (!strcmp(checkSym(line[r + 1]), "APOSTROPHE")) r += 2;
					else {
						errorOut(1);
						r++;
					}
