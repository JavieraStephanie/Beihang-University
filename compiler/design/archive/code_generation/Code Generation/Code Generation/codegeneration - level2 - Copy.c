...
int lc = 0, l = 0, r = 0, ai = 0, bi = 1, ci = 0, ci0, ci1 = 0, c1 = 0, c2 = 0, ti = 28, ti0, di, level = 0, address = 3, pos, pos0 = 0, vali, num = 0, num2;
...
int cstart = 0, varstart = 0, forstart = 0;
float valf = 0.0;
...
{
	if (!strcmp(line, "INT")) return 1;
	else if (!strcmp(line, "REAL")) return 2;
	else if (!strcmp(line, "BOOLEAN")) return 3;
	else if (!strcmp(line, "STRING")) return 4;
	else if (!strcmp(line, "ARRAY")) return 5;
	return 0;
}
	...
	if (!strcmp(line2, "LPAREN")) {
		if (line[r] == '\'' || line[r + 1] == '\'') {
			if (!strcmp(parse(), "STRING")) {
				storeCode(ldc, 0, strlen(valc));
				storeCode(wrs, 0, 0);//inum
				strcpy(valc, "\0");
				line2 = parse();
	...
	if (!strcmp(cline, "IDEN")) {
		...
		if (!strcmp(valtab[pos].kind, "constant")) {
			if (!strcmp(valtab[pos].type, "REAL"))
				storeCode(ldr, 0, 0);
			else storeCode(ldc, 0, 0);
		}
		else if (!strcmp(valtab[pos].kind, "variable")) {
			/*if it's an array then use these commands
				if normal
                then f := 0
                else f := 1;
...
void checkAssign(char* line2)
{
	...
	if (!strcmp(line2, "LPAREN") && assign == 0) {
		...
		if (valtab[pos0].normal) storeCode(lda, 0, 0);
		else storeCode(lod, 0, 1);
		...
		if (!strcmp(valtab[pos0].type, restyp)) {
			/*if it's an array do this command
		...
		if (assign == 0) {
			if (!strcmp(valtab[pos].kind, "procedure"))
				storeCode(cal, btab[valtab[pos].ref].psize - 1, 0);
...
void interpret()
{
	...
	do {
		...
		break; case wrw: chrcount = chrcount + fld[ir.a]; // write1
		{
			...
			default:
			break; case 1: printf("%*d", fld[1], s[t].i);
			break; case 2: printf("%*d", fld[2], s[t].r);
...
int main() {
	...
	printf("\tname\t\tlink\tobj\ttyp\tnrm\tlev\tadr\n");
	for (k = 28; k < ti+1; k++) {
		printf("%d\t%s\t\t%d\t%d\t%d\t%d\t%d\n", k, valtab[k].name, valtab[k].link, checkKind(valtab[k].kind), checkType(valtab[k].type), valtab[k].normal, valtab[k].level);
	}
