...
<变量说明>              ::=  <标识符>{, <标识符>} :  <类型>
<类型>                      ::=   <基本类型>|array'['<无符号整数>']' of <基本类型>    //此处<无符号整数>表示数组元素个数，其值需大于0
<基本类型>              ::=   integer | real | char
<过程说明部分>     ::=  <过程首部><分程序>;
<函数说明部分>     ::=  <函数首部><分程序>;
<过程首部>             ::=   procedure<标识符>'('[<形式参数表>]')';   //<形式参数表>是否出现分别对应了有无参数的情况
<函数首部>               ::=   function <标识符>'('[<形式参数表>]')': <基本类型>;
<形式参数表>          ::=  <形式参数>{; <形式参数>}
<形式参数>          ::= [var] <标识符>{, <标识符>}: <基本类型>
...
（4）写语句中，字符串原样输出，表达式只有单个字符类型的变量或常量时按字符输出，其他表达式均按整型或实型输出

（5）数组的下标从0开始

（6）for循环语句步长为1

（7）带var的形式参数表示变量形参，参数传递方式是传地址，对应的实参只能是变量