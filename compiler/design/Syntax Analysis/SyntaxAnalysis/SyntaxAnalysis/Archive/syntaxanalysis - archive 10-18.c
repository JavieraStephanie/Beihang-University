...
FILE* fi, * fo;
size_t templen;
bool mainFunc = false, neg = false, assign = false, increment = false, delay = false, delaysym = false, valfunc = true;
		...
		if (!strcmp(parseWord(), "ASSIGN")) {
			if (!strcmp(type, "INTTK")) {
				...
				if (!strcmp(line, "MINU") || !strcmp(line, "PLUS")) {
					line = parseWord();
					delay = false;
					if (!strcmp(line, "INTCON")) printOutput(dcode, dcontent);
				}
...
void checkFunc(char* line)
{
	if (isType(line)) {
		...
		valfunc = false;
		line = parseWord();
		if (!strcmp(line, "IDENFR")) storeFunc(subStr);
		else if (!strcmp(line, "MAINTK")) mainFunc = true;
		else return;
	}
	...
	while (r <= len && l <= r) {
		...
		if (isSymbol(line[r]) && l == r) {
			...
			if (strcmp(checkSym(line[r - 1]), "QUOTE") && str != 0) {
				...
				else if (!strcmp(checkSym(line[r - 1]), "QUOTE")) {
					strcpy(lastsymbol, "QUOTE");
				}

				else {
					if (delaysym && !isOp(checkSym(line[r - 1]))) delayedOutput(checkSym(line[r - 1]), toString(line[r - 1]));
