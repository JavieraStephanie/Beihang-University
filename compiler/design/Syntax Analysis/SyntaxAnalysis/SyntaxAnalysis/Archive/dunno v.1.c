...
FILE* fi, * fo;
size_t templen;
bool normal = false, mainFunc = false, neg = false, assign = false, increment = false, delay = false, delaysym = false, valfunc = true;
	...
	if (!strcmp(line, "IDENFR")) {
		...
		if (!strcmp(parseWord(), "ASSIGN")) {
			...
			if (!strcmp(type, "INTTK")) {
				...
				else if (!strcmp(line, "INTCON")) printOutput(dcode, dcontent);
			}
...
void checkParam(char* line)
{
	if (isType(line)) {
		printOutput(dcode, dcontent);
		if (!strcmp(parseWord(), "IDENFR")) printOutput(dcode, dcontent);;
