...
char *subStr, lastsymbol[10], tempstr[100], line[100], chstr[2], op[5] = "PLUS", op2[5] = "\0", dcontent[10] = "\0", dcode[10] = "\0", funcname[20] = "\0";
...
void checkContent(char* line)
{
	if (!strcmp(line, "LBRACE")) {
		...
		if (!strcmp(line, "RBRACE")) {
			...
			else if (valfunc) printSyntax("<End of value function declaration>");
			else printSyntax("<End of non-value function declaration>");
			valfunc = true;
		}
...
char* checkStatement(char* line)
{
	...
	//Assignment statement
		...
		if (!strcmp(line, "LPARENT")) {
			...
			if (!strcmp(line, "\0") && !delay) line = parseWord();
		}
