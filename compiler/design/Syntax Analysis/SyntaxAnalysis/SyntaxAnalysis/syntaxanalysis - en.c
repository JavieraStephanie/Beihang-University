	...
	if (!strcmp(line, "IDENFR")) {
		...
		if (!strcmp(parseWord(), "ASSIGN")) {
			...
			if (!strcmp(line, "COMMA")) {
				...
				printSyntax("<Constant Declaration>");
				printOutput(dcode, dcontent);
			}
		}
	}
...
void checkVar(char* line, char* type)
{
	if (!strcmp(line, "IDENFR")) {
		...
		if (!strcmp(line, "LPARENT")) {
			if (hasvar) printSyntax("<Variable Statement>");
			printOutput(dcode3, dcontent3);
			printOutput(dcode2, dcontent2);
			storeFunc(subStr);
			printSyntax("<Function Header>");
			printOutput(dcode, dcontent);
			hasvar = false;
			checkFunc(line);
			return;
		...
		if (!strcmp(line, "COMMA")) {
			...
			printOutput(dcode3, dcontent3);
			printOutput(dcode2, dcontent2);
			printSyntax("<Variable Declaration>");
			printOutput(dcode, dcontent);
			normal = false;
			hasvar = true;
		}
...
void checkFunc(char* line)
{
	if (!strcmp(line, "IDENFR") || !strcmp(line, "MAINTK")) {
		...
		if (!strcmp(line, "MAINTK")) mainFunc = true;
			...
			storeFunc(subStr);
			if (valfunc) printSyntax("<Function Header>");
		}
		line = parseWord();
	}
	...
	if (!strcmp(line, "LPARENT")) {
		...
		if (!strcmp(line, "RPARENT")) {
			if (!mainFunc) printSyntax("<Parameters Table>");
			printOutput(dcode, dcontent);
			line = parseWord();
...
void checkContent(char* line)
{
	if (!strcmp(line, "LBRACE")) {
		...
		if (!strcmp(line, "RBRACE")) {
			...
			printOutput(dcode, dcontent);
			if (mainFunc) printSyntax("<Main Function>");
			else if (valfunc) printSyntax("<Value Function Declaration>");
			else printSyntax("<Non-Value Function Declaration>");
			valfunc = true;
		}
	}
...
char* checkStatement(char* line)
{
	...
	//Function call && Assignment statement
		...
		if (!strcmp(line, "LPARENT")) {
			...
			if (!strcmp(line, "RPARENT")) {
				printOutput(dcode, dcontent);
				if (valtab[findId()].valfunc) printSyntax("<Value Function Call>");
				else printSyntax("<Non-Value Function Call>");
				line = parseWord();
			}
		}
						...
						if (line[r] == '[' && assign) {
							...
							if (valtab[findId()].valfunc) {
								...
								if (!strcmp(checkFuncCall(parseWord()), "RPARENT")) {
									printOutput(dcode, dcontent);
									printSyntax("<Value Function Call>");
								}
							}
							else {
								parseWord();
