...
int lc = 0, l = 0, r = 0, ci = 0, ci0, ci1 = 0, ti = 1, ti0, di, level = 0, adr = 0, pos, pos0 = 0;
char lastsymbol[10], line[100], id[30] = "\0", vartyp[10] = "\0", val[15] = "\0";
int begin = 0, cond = 0, repeat = 0, forloop = 0, assign = 0, str = 0, con = 0, var = 0, relat = 0;
...
void errorOut(int errnum)
{
	printf("(L: %d C: %d)", lc, r);
	switch (errnum) {
	case 1: printf("= expected instead of :="); break;
	case 2: printf("Number expected after ="); break;
...
bool isOp(char* line)
{
	if (!strcmp(line, "DIV") || !strcmp(line, "TIMES") || !strcmp(line, "MINUS") || !strcmp(line, "PLUS")) {
		if (!strcmp(line, "DIV")) storeCode(opr, 0, 5);
		else if (!strcmp(line, "TIMES")) storeCode(opr, 0, 4);
		else if (!strcmp(line, "MINUS")) storeCode(opr, 0, 3);
		else if (!strcmp(line, "PLUS")) storeCode(opr, 0, 2);
		return (true);
	}
	return (false);
}
...
bool checkAssign(char* line)
{
	...
	if (!strcmp(line, "LPAREN") && assign == 0) {
		...
		if (!strcmp(line, "IDEN")) {
			...
			strcpy(valtab[pos0].val, valtab[pos].val);
		}
		else if (!strcmp(line, "INT")) {
			storeCode(lit, 0, atoi(val));
			strcpy(valtab[pos0].val, val);
		}
		line = parse();
		if (!strcmp(line, "LBRACKETS")) {
			return checkAssign(parse());
