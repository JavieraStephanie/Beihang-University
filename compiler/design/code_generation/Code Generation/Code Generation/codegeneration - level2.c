...
int lc = 0, l = 0, r = 0, ai = 0, bi = 1, ci = 0, ci0, ci1 = 0, ci2 = 0, c1 = 0, c2 = 0, ti = 27, ti0, level = 1, address = 5, pos, pos0 = 0, vali;
char lastsymbol[10], line[100], id[30] = "\0", vartyp[10] = "\0", valc[15] = "\0", valc2, op[10] = "PLUS", op2[10] = "\0", count[50];
...
bool stackdump, forbegin = false, upstart = false, downstart = false, negative = false,
negativewhole = false, cntop = false, gotnum = false, valpar = false, parcheck = false,
isfun = false, ifthen = false, ifelse = false, isarray = false;
int display[7], chrcount, lncount, ocnt, prb, prt, strcount = 0, sx = 0, errcnt = 0,
procbegin, funcbegin;
float rconst[20];
...
{
	switch (errnum) {
	case 0: printf("Identifier not defined"); break;
	case 1: printf("Identifier redefinition"); break;
	case 2: printf("Identifier expected"); break;
	case 3: printf("Program should started with 'program'"); break;
	case 4: printf("')' expected"); break;
	case 5: printf("':' expected when defining a variable type"); break;
	case 6: printf("Invalid symbol, some symbols will be skipped"); break;
	case 7: printf("Parameters should start with an identifier or 'var'"); break;
	case 8: printf("'of' expected"); break;
	case 9: printf("'(' expected"); break;
	case 10: printf("Type should start with identifier, array, or record"); break;
	case 11: printf("'[' expected"); break;
	case 12: printf("']' expected"); break;
	case 13: printf("'..' expected"); break;
	case 14: printf("';' expected"); break;
	case 15: printf("Function return type should be integer, real, boolean, or char"); break;
	case 16: printf("'=' expected, ':=' should only be used in assignment statements not declarations"); break;
	case 17: printf("Expression following 'if', 'while', or 'until' should be boolean type"); break;
	case 18: printf("The loop variable after for can only be integer, char or boolean"); break;
			...
			if (!strcmp(parse(), "INT")) {
				...
				if (!strcmp(parse(), "RBRACKETS")) {
					if (!strcmp(parse(), "OFSYM")) {
						...
						if (isType(line)) {
							int elsz = 0;
							if (!strcmp(line, "INTTYPE")) elsz = sizeof(int);
							else if (!strcmp(line, "REALTYPE")) elsz = sizeof(float);
							else if (!strcmp(line, "CHARTYPE")) elsz = sizeof(char);
...
bool checkParam(char* line)
{
	...
	if (!strcmp(line, "SEMICOLON") || !strcmp(line, "RPAREN")) {
		...
		else if (!strcmp(line, "RPAREN")) { printf("Here\n"); return (true); }
	}
		...
		if (line[r] == '\'' || line[r + 1] == '\'' || line[r] == '"' || line[r + 1] == '"') {
			if (!strcmp(parse(), "STRING")) {
				...
				if (!strcmp(line2, "RPAREN")) {
					if (!strcmp(restyp, "ARRAY")) strcpy(restyp, "INT");
...
int factor()
{
	strcpy(cline, parse());
	cntop = false;
	if (!strcmp(cline, "IDEN") || !strcmp(cline, "ARRAY")) {
		pos = findId(id);
		if (pos == 0) {
			strcpy(id, "\0");
			return (false);
		...
		if (!strcmp(valtab[pos].kind, "constant")) {
			...
			if (!strcmp(valtab[pos].type, "ARRAY")) {
				...
				if (strcmp(atab[a].inxtyp, valtab[pos].type)) storeError(26);
		...
		else if (!strcmp(valtab[pos].kind, "procedure")) {
			storeError(44);
		}
		strcpy(id, "\0");
...
int expression()
{
	...
	cstart = 0;
	gotnum = negative = negativewhole = false;
	return result;
}
...
void checkAssign(char* line2)
{
	...
	if (!strcmp(line2, "LPAREN") && assign == 0) {
		...
		else if (!strcmp(valtab[pos0].type, "ARRAY")) {
			storeCode(sto, 0, 0);
			storeCode(cpb, 0, atab[valtab[pos0].ref].size);
		}
...
void checkConstant(char* line)
{
	if (!strcmp(line, "IDEN")) {
		...
		else if (!strcmp(line, "BECOMES")) {
			storeError(16);
			resetData();
			strcpy(ctype, "NOTYPE");
			line = parse();
			while (strcmp(line, "COMMA") && strcmp(line, "SEMICOLON")) line = parse();
			if (!strcmp(line, "COMMA")) checkConstant(parse());
		}
	}
	else storeError(2);
	return;
			...
			if (isType(line) || isArray(line)) {
				...
				for (; varstart < ti + 1; varstart++) {
					if (!strcmp(line, "INTTYPE")) strcpy(valtab[varstart].type, "INT");
						...
						strcpy(valtab[varstart].type, "ARRAY");
						valtab[varstart].ref = aref;
					}
				}
				varstart = 0;
				strcpy(ctype, "NOTYPE");
...
char* parse() {
	...
	while (r <= len && l <= r) {
		if (!isSymbol(line[r])) r++;

		if (isSymbol(line[r]) && l == r) {
			l = ++r;

			if (str > 0 && line[r - 1] != '\'' && line[r - 1] != '"') {
				if (!strcmp(lastsymbol, "CONSTSYM"))
					valc2 = line[l];
				else append(valc, line[r - 1]);
			}

			else if (!strcmp(checkSym(line[r - 1]), "LSS")) {
				...
				if (((line[r] > '0' && line[r] < '9') || line[r] == '(') && (!strcmp(lastsymbol, "CONSTSYM") || cntop || !gotnum)) {
					if (line[r] == '(') {
						if (negativewhole) negativewhole = false;
						else negativewhole = true;
					}
					else {
			...
			else if (!strcmp(checkWord(subStr), "DOSYM") || !strcmp(checkWord(subStr), "TOSYM") ||
				!strcmp(checkWord(subStr), "DOWNTOSYM") || !strcmp(checkWord(subStr), "THENSYM") ||
				!strcmp(checkWord(subStr), "ARRTYPE") || !strcmp(checkWord(subStr), "OFSYM") ||
				isType(checkWord(subStr)))
			...
			if (!strcmp(lastsymbol, "PROGSYM")) {
				...
				if (repeat) {
					if (checkCondition()) {
						...
						if (upstart && forbegin) {
							storeCode(f2u, 0, ci2);
							code[ci1].a = ci;
							upstart = forbegin = false;
							ci1 = ci2 = 0;
						}
						else if (downstart && forbegin) {
							storeCode(f2d, 0, ci2);
							code[ci1].a = ci;
							downstart = forbegin = false;
							ci1 = ci2 = 0;
						}
						else if (ifthen) {
							while (isspace(line[r])) r++;
				...
				if (!strcmp(parse(), "IDEN")) {
					...
					strcpy(id, "\0");
					if (strcmp(valtab[pos].kind, "variable")) {
						if (!valtab[pos].normal) storeError(37);
						else storeCode(lda, valtab[pos].level, valtab[pos].adr);
						if (!isConstVal(valtab[pos].type)) storeError(18);
						//return (false);
					}
					if (!strcmp(parse(), "BECOMES")) {
						char* downto;
						cstart++;
						expression(); while (isspace(line[r - 1])) r--;
						downto = parse();
						if (!strcmp(downto, "TOSYM") || !strcmp(downto, "DOWNTOSYM")) {
							cstart++;
							expression(); while (isspace(line[r - 1])) r--;
							ci1 = ci;
							if (!strcmp(downto, "TOSYM")) {
								upstart = true;
								storeCode(f1u, 0, 0);
							}
							else {
				...
				begin++;
				if (begin == procbegin || begin == funcbegin)
				{
					valtab[prt].adr = ci;
				}
				/*if (level > 1) {
...
void interpret()
{
	...
	do {
		...
		break; case jmp: p = ir.a; // jump
		break; case jpc: // conditional jump
		{
			if (!s[t].b) p = ir.a;
			t = t - 1;
		}
	...
	for (k = 0; k < errcnt; k++) {
		printf("At line: %d column: %d ", errtab[k].line, errtab[k].col);
		errorOut(errtab[k].n);
	}
}
...
int main() {
	...
	printf("arrays\txtyp\tetyp\teref\tlow\thigh\tesize\tsize\n");
	for (k = 1; k < ai + 1; k++) {
		printf("%d\t%s\t%d\t%d\t%d\t%d\t%d\t%d\n", k, atab[k].inxtyp, checkType(atab[k].eltyp), atab[k].elref, atab[k].low, atab[k].high, atab[k].elsize, atab[k].size);
	}
