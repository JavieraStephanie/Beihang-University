...
char* subStr, assigniden[30], tempstr[100], stringval[100], charval[2], arrlimit[10], lineo[200], chstr[2], op[5] = "PLUS", op2[5] = "\0", funcname[20] = "\0", functype[10] = "\0", idenname[20] = "\0", identype[10] = "\0";
...
void storeVal(char* name, char* val)
{
	...
	if (!valtab[valcounter].idenvar) {
		if (!strcmp(valtab[valcounter].identype, "CHARTK")) {
			...
			storeData(valtab[valcounter].name, 5, 1, tempstr);
		}
		else {
			if (neg) {
				snprintf(tempstr, sizeof(tempstr), "-%s", valtab[valcounter].val);
				storeData(valtab[valcounter].name, 2, 0, tempstr);
			}
			else storeData(valtab[valcounter].name, 2, 0, valtab[valcounter].val);
		}
	}
	else {
		if (!strcmp(valtab[valcounter].identype, "CHARTK")) {
...
{
	char tempd[30];
	if (isparam) snprintf(tempd, sizeof(tempd), "$s%d", tempid);
	else {
