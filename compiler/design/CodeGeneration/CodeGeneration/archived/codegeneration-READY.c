...
FILE* fi, * fo;
size_t templen;
bool arr = false, expimm = true, resimm = false, arrimm = false, imm = false, ret = false, arraysize = false, hasvar = false, mainFunc = false, neg = false, assign = false, valfunc = true, idenvar = false;
int id = -1, varnum = 0, paramnum = 0, newline = 0, lc = 1, l = 0, r = 0, len = 0, str = 0, num = 0, intcon = 0, layer = 0, funccounter = 0, valcounter = 0;
int funcid[10], fid = 0, idenid[10], iid = 0, intExp[10], iei = 0, regnum = 0, labelnum = 0, arrreg = 0;
char* subStr, assigniden[30], tempstr[100], charval[2], arrlimit[10], line[200], chstr[2], op[5] = "PLUS", op2[5] = "\0", funcname[20] = "\0", functype[10] = "\0", idenname[20] = "\0", identype[10] = "\0";
char * expline, countop[5], relatop[3], countval[30], cond[50], init[30], incre[30];
...
char* checkStatement(char* line)
{
	...
	//Function call && Assignment statement
		...
		if (!strcmp(line, "LPARENT")) {
			...
			if (!strcmp(line, "LBRACK")) {
				...
				if (imm) snprintf(tempstr, sizeof(tempstr), "[%s]", countval);
				else snprintf(tempstr, sizeof(tempstr), "[t%d]", regnum - 1);
				regnum = 0; imm = expimm = resimm = false;
				intExp[iei] = 0;
				assign = false;
				iid--;
	...
	//Return Statement
		...
		if (!strcmp(line, "LPARENT")) {
			...
			if (imm) printf(" %s", countval);
			else printf(" t%d", regnum - 1);
			regnum = 0; imm = expimm = resimm = false;
			intExp[iei] = 0;
			assign = false;
			if (!strcmp(line, "RPARENT")) {
...
char* checkCondition(char* line)
{
	...
	if (isConstVal(line) || !strcmp(line, "LPARENT")) {
		...
		intExp[iei] = 0;
		if (!imm) snprintf(cond, sizeof(cond), "t%d ", regnum - 1);
		else strcpy(cond, countval);
		imm = expimm = resimm = false;
	}
	else errorOut(1);
	...
	if (!strcmp(line, "IDENFR")) {
		...
		if (!strcmp(parseWord(), "ASSIGN")) {
			...
			if (!strcmp(parseWord(), "IDENFR")) {
				...
				if (!strcmp(line, "PLUS") || !strcmp(line, "MINU")) {
					newline = 0;
					if (!strcmp(line, "PLUS")) strcat(incre, " + ");
					else strcat(incre, " - ");
	...
	if (isConstVal(line) || !strcmp(line, "LPARENT") || !strcmp(line, "PLUS") || !strcmp(line, "MINU")) {
		while (strcmp(line, "RPARENT")) {
			...
			if (isConstVal(line) || !strcmp(line, "LPARENT")) {
				...
				paramnum++; regnum = 0; imm = expimm = resimm = false;
				intExp[iei] = 0;
				if (!strcmp(line, "COMMA")) {
					newline = 0;
					line = parseWord();
	...
	if (!strcmp(line, "LPARENT")) {
		...
		if (!strcmp(line, "STRCON")) {
			...
			if (!strcmp(line, "COMMA")) {
				...
				line = expline; regnum = 0; imm = expimm = resimm = false;
			}
		}
...
int checkExpression(char* line)
{
	int term, tempres, tempreg = regnum, res = checkTerm(line);
	char tempval[30];
	if (imm) expimm = true;
	tempres = num; strcpy(tempval, countval);
	...
	if (isOp(expline)) {
		while (!strcmp(countop, "PLUS") || !strcmp(countop, "MINU")) {
			if (!strcmp(countop, "PLUS")) {
				...
				printf("t%d = ", regnum++);
				if (expimm) printf("%s", tempval);
				else printf("t%d", tempreg);
				printf(" + ");
				if (imm) printf("%s\n", countval);
				else printf("t%d\n", regnum - 2);
				res += term;
...
int checkTerm(char* line)
{
	int fact, tempres, tempreg = regnum, res = checkFactor(line);
	char tempval[30];
	if (imm) resimm = true;
	tempres = num; strcpy(tempval, countval);
	line = parseWord();
					...
					if (isIden(subStr) && !isSymbol(line[r - 1])) {
						if (line[r] == '[' && assign) {
							...
							if (findId() < 0 && findParam(functab[layer - 1].paramnum, layer - 1) < 0) {
								...
								iei++;
								checkExpression(parseWord());
								printf("t%d = %s", regnum++, arrayname);
								if (imm) printf("[%s]\n", countval);
								else printf("[t%d]\n", regnum - 2);
								snprintf(countval, sizeof(countval), "t%d", regnum - 1);
								temp = expline;
								if (intExp[iei] == 0) errorOut(9);
