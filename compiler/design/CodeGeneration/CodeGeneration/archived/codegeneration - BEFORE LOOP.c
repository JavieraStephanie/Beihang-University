...
char * expline, countop[5], relatop[3], countval[30], cond[50], init[30], incre[30], increreg[5], increop[5];
...
char* checkStatement(char* line)
{
	...
	int label, nextlabel, tempid, tempcondcounter, tempincreid, tempincrenum;
	bool isparam = false, tempincreparam;
	...
	//For
		...
		if (!strcmp(line, "LPARENT")) {
			...
			fprintf(fm, "%s\n", init);
			strcpy(temprelatop, relatop); strcpy(tempcond1, cond1); strcpy(tempcond2, cond2);
			strcpy(increment, incre); tempincreid = increid; tempincreparam = increparam; tempincrenum = increnum;
			tempcondcounter = condcounter; condcounter = 0;
			fprintf(fm, "GOTO LABEL %d\n", label);
			snprintf(d, sizeof(d), "LABEL%d", label);
			storeCode3("b", d);
			...
			if (!strcmp(increop, "PLUS")) snprintf(s, sizeof(s), "%d", tempincrenum);
