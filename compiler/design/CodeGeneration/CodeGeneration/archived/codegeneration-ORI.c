...
char * expline, countop[5], relatop[3], countval[30], cond[50], init[30], incre[30], increreg[5], increop[5], increnum[10];

//MIPS DATA COUNTER
bool syscall = false, iscond = false, func1 = false;
int strnum = 0, datacounter = 0, codecounter = 0, maincounter = 0, vars = 0, arrId = 0, condcounter = 0;
char mipsData[100][200], mipsCode[1000][100], mainCode[1000][100], dataAddr[10], curType[10], curIndex[20];
char d[50], s[50], t[50], cond1[5], cond2[5], condstring[50][50];
...
void makeBranch(char* cond1, char* relatop, char* cond2, char* label, int condcounter);
...
{
	char temp[200];
	snprintf(temp, sizeof(temp), "%s: %s %s", name, dataType(type), content);

	strcpy(mipsData[datacounter++], temp);
...
void makeBranch(char* cond1, char* relatop, char* cond2, char* label, int condcounter)
{
	...
	else if (!strcmp(relatop, "<")) {
		snprintf(d, sizeof(d), "$t%d", regnum++);
		storeCode("slt", d, cond1, cond2);
		storeCode("bne", "$0", d, label);
	}
	else if (!strcmp(relatop, ">")) {
		snprintf(d, sizeof(d), "$t%d", regnum++);
		storeCode("slt", d, cond1, cond2);
		storeCode("beq", "$0", d, label);
	}
	else if (!strcmp(relatop, "<=")) {
		snprintf(d, sizeof(d), "$t%d", regnum++);
		storeCode("slt", d, cond1, cond2);
		storeCode("bne", "$0", d, label);
		storeCode("beq", cond1, cond2, label);
	}
	else if (!strcmp(relatop, ">=")) {
		snprintf(d, sizeof(d), "$t%d", regnum++);
		storeCode("slt", d, cond1, cond2);
...
char* checkStatement(char* line)
{
	char temprelatop[50], increment[30], arrindex[5], tempcond1[50], tempcond2[50];
	int label, nextlabel, tempid, tempcondcounter;
	bool isparam = false;
	...
	//If-else
		...
		if (!strcmp(line, "LPARENT")) {
			...
			tempcondcounter = condcounter; condcounter = 0;
			intExp[iei] = 0;
			assign = false;
			...
			makeBranch(tempcond1, reverseRelat(temprelatop), tempcond2, tempstr, tempcondcounter);
			labelnum++;
	...
	//Do-While 
		...
		if (!strcmp(line, "WHILETK")) {
			...
			if (!strcmp(parseWord(), "LPARENT")) {
				...
				makeBranch(tempcond1, temprelatop, tempcond2, tempstr, tempcondcounter);
			}
			else errorOut(1);
		}
		else errorOut(14);
	...
	//For
		...
		if (!strcmp(line, "LPARENT")) {
			...
			fprintf(fm, "%s\n", increment);
			if (!strcmp(increop, "PLUS")) storeCode("addi", increreg, increreg, increnum);
			else storeCode("sub", increreg, increreg, increnum);
			fprintf(fm, "Label %d :\n", label);
			snprintf(d, sizeof(d), "LABEL%d:", label);
			storeCode4(d);
			fprintf(fm, "%s\n", temprelatop);
	...
	//Function call && Assignment statement
		...
		if (!strcmp(line, "LPARENT")) {
			...
			if (findFunc() < 0) {
				...
				snprintf(d, sizeof(d), "LABEL%d", functab[funcid[fid - 1]].labelnum);
				storeCode3("b", d);

				snprintf(t, sizeof(t), "-%d", (stacknum+1)*4);
				storeCode("addi", "$sp", "$sp", t);

				for (int i = stacknum; i > 0; i--) {
					snprintf(s, sizeof(s), "$s%d", i-1);
					snprintf(d, sizeof(d), "%d($sp)", i * 4);
					storeCode2("sw", s, d);
				}
				storeCode2("sw", "$ra", "0($sp)");

				snprintf(t, sizeof(t), "LABEL%d", functab[funcid[fid - 1]].labelnum);
				storeCode3("jal", t);

				for (int i = functab[funcid[fid - 1]].paramnum; i > 0; i--) {
					snprintf(s, sizeof(s), "$s%d", i - 1);
					snprintf(d, sizeof(d), "%d($sp)", i * 4);
					storeCode2("lw", s, d);
				}
				storeCode2("lw", "$ra", "0($sp)");

				snprintf(t, sizeof(t), "%d", (functab[funcid[fid - 1]].paramnum + 1) * 4);
				storeCode("addi", "$sp", "$sp", t);
				intExp[iei] = 0;
				assign = false;
			...
			if (!strcmp(line, "ASSIGN")) {
				...
				if (isparam) {
					snprintf(d, sizeof(d), "$s%d", tempid);
				}
				else {
					if (valtab[tempid].arr) {
						storeCode("add", "$s7", "$0", arrindex);
						strcpy(d, assigniden);
						strcat(d, "($s7)");
					}
					else {
				...
				if (imm) {
					if (!func) {
						...
						snprintf(s, sizeof(s), "$v0");
				}
				else snprintf(s, sizeof(s), "$t%d", regnum - 1);

				if (negExp) storeCode2("neg", s, s);
				fprintf(fm, "%s = %s\n", d, s);
				if (isparam) storeCode2("move", d, s);
				else {
	...
	//Return Statement
		...
		if (!strcmp(line, "LPARENT")) {
			...
			if (imm) {
				fprintf(fm, " %s", countval);
				getReg(countval, curType);
				snprintf(s, sizeof(s), "$t%d", regnum - 1);
				if (func) strcpy(s, "$v1");
			}
			else {
				fprintf(fm, " t%d", regnum - 1);
				snprintf(s, sizeof(s), "$t%d", regnum - 1);
			}
	...
	if (!strcmp(line, "IDENFR")) {
		...
		if (!strcmp(parseWord(), "ASSIGN")) {
			...
			if (!strcmp(parseWord(), "IDENFR")) {
				...
				if (!strcmp(line, "PLUS") || !strcmp(line, "MINU")) {
					newline = 0;
					if (!strcmp(line, "PLUS")) strcat(increop, "PLUS");
					else strcat(increop, "MINU");
					if (!strcmp(parseWord(), "INTCON")) {
						newline = 0;
						snprintf(tempstr, sizeof(tempstr), "%d", intcon);
						strcat(incre, tempstr);
						strcpy(increnum, tempstr);
						line = parseWord();
					}
					else errorOut(1);
				}
...
int checkExpression(char* line)
{
	...
	char tempval[30], temptype[10], d[30];
	bool tempneg = false;
	if (imm) expimm = true;
	if (neg) tempneg = true;
	tempres = num; tempreg = regnum - 1;
	strcpy(tempval, countval); strcpy(temptype, curType);
	if (func) tempstat = 1;
	...
	if (isOp(expline)) {
		while (!strcmp(countop, "PLUS") || !strcmp(countop, "MINU")) {
			...
			if (!strcmp(countop, "PLUS")) {
				...
				if (expimm) {
					if (tempstat == 1) strcpy(s, "$v0");
						...
						else snprintf(s, sizeof(s), "$t%d", regnum - 2);
					}
				}
				...
				else snprintf(t, sizeof(t), "$t%d", regnum - 2 - regoffset);
				...
				if (expimm) {
					if (tempstat == 1) strcpy(s, "$v0");
						...
						else snprintf(s, sizeof(s), "$t%d", regnum - 2 - arr);
					}
				}
				else snprintf(s, sizeof(s), "$t%d", tempreg);
			...
			imm = expimm = func = arr = tempneg = neg = false;
			tempreg = regnum - 1;
		}
	}
	...
	if (isOp(line)) {
		while (!strcmp(countop, "MULT") || !strcmp(countop, "DIV")) {
			...
			if (!strcmp(countop, "MULT")) {
				snprintf(d, sizeof(d), "$t%d", regnum++);
				if (regnum == 10) regnum = 0;  func = false;
				if (tempstat == 1) storeCode2("move", "$v0", "$v1");
			...
			imm = resimm = func = arr = tempneg = neg = false; 
			tempreg = regnum - 1;
			line = parseWord();
			isOp(line);
		}
					...
					if (isIden(subStr) && !isSymbol(line[r - 1])) {
						if (line[r] == '[' && assign) {
							...
							if (findId() < 0 && findParam(functab[layer - 1].paramnum, layer - 1) < 0) {
								...
								tempreg = regnum;
								checkExpression(parseWord());
								//fprintf(fm, "t%d = %s", regnum++, arrayname);
								if (regnum == 10) regnum = 1;
								else regnum++;
								if (imm) {
									if (arr) {
								...
								snprintf(countval, sizeof(countval), "$t%d", regnum - 2);

								snprintf(tempstr, sizeof(tempstr), "$t%d", regnum - 2);
