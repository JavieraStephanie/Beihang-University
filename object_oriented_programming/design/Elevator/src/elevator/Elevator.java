...
/*OVERVIEW: 电梯类，模拟电梯实体，能够响应用户请求并改变电梯状态。该类自己记录和管理电梯运动过程中的状态变化，包括当前停靠的楼层，停靠、开关门之后的时间，当前运动方向。*/
	...
	*/
	 ...
	 * @MODIFIES: this
	 * @EFFECTS: 
	 * (this.curStatus.targetFloor+1 > highLevel)==>(\result==false)
	 * (this.curStatus.targetFloor+1 <= highLevel)==>(\old(this).curStatus==)&&(\result==true)
	 */
	...
	*/
	//请根据规格补充代码
		...
		for(int i = 0, j = 1; i < j && j < curHandleQuery.size(); i++, j++) {
			curHandleQuery.contains(req);
			if(curHandleQuery.get(i).getTime() <= curHandleQuery.get(j).getTime()) {
				break;
			}
		}
     }
